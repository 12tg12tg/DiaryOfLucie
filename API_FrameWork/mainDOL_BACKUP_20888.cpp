...
void mainDOL::render()
{
	...
	case mainDOL::DOLSTATE::GETSKILL:
	{
		...
		//------------------------------------½ÇÁ¦Ãâ·Â------------------------------------------
		//1. Zorder ÀÏ°ı "DolDC"¿¡Ãâ·Â
		ZORDER->ZorderTotalRender(getDolDC());
		//2. DolDC¸¦ Ä«¸Ş¶óÀû¿ëÇÏ¿© SCORPDC¿¡ Ãâ·Â
	...
	//-----------------------------------------Å×½ºÆ®-----------------------------------------
	//TCHAR str[128];
	//wsprintf(str, "Ä«¸Ş¶ó ·¹ÇÁÆ®/Å¾ : %d, %d", CAMERA->getRect().left, CAMERA->getRect().top);
...
void mainDOL::gameInit()
{
	...
	_mapm->setSkillMemoryLink(_sk);				//¸Ê¿¡¼­ ½ºÅ³
...
{
	SOUND->addSound("¹è°æÀ½¾Ç", "sound/stage_forest.ogg", true, true);
	SOUND->addSound("º¸½ºÀ½¾Ç", "sound/boss_forest.ogg", true, true);
	SOUND->addSound("¸¶Áö¸·º¸½ºÀ½¾Ç", "sound/lastboss.ogg", true, true);
	
	SOUND->addSound("´Ş¸®´Â¼Ò¸®", "sound/footstep_grass_run_01.ogg", false, false);
	SOUND->addSound("°È´Â¼Ò¸®", "sound/footstep_grass_walk_04.ogg", false, false);

	SOUND->addSound("½½¶óÀÓ¼Ò¸®", "sound/slime1.ogg", false, false);
	SOUND->addSound("½½¶óÀÓÁ×´Â¼Ò¸®", "sound/slime_dead.ogg", false, false);
	SOUND->addSound("¹ìÁ×´Â¼Ò¸®", "sound/snaby_dead.ogg", false, false);
	SOUND->addSound("¹ì¼Ò¸®", "sound/snaby1.ogg", false, false);
	SOUND->addSound("¹ö¼¸Á×´Â¼Ò¸®", "sound/mushman_dead.ogg", false, false);
	SOUND->addSound("¿äÁ¤Á×´Â¼Ò¸®", "sound/fairy_dead.ogg", false, false);
	SOUND->addSound("²ÉÁ×´Â¼Ò¸®", "sound/se_dead1.ogg", false, false);
	SOUND->addSound("¹ö¼¸¼Ò¸®", "sound/mushman1.ogg", false, false);

	SOUND->addSound("Ä®¼Ò¸®", "sound/se_sword1.ogg", false, false);
	SOUND->addSound("È°¼Ò¸®", "sound/bow1.ogg", false, false);
	SOUND->addSound("°ø°İ¼Ò¸®", "sound/atk602.ogg", false, false);

	SOUND->addSound("ÄÚÀÎ", "sound/Coin.ogg", false, false);
	SOUND->play("¹è°æÀ½¾Ç",0.05);
}

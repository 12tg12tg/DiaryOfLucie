...
void Cskill::uirender()
{
	...
	for (size_t i = 0; i < skillnum; i++)
	{
		...
		ZORDER->UIFrameRender(IMAGE->findImage("스킬이미지"), ZUISECOND, 99, skillUI[i].left, skillUI[i].top,
        ...
        if (storage[i].isCool)
        {
            ...
            ZORDER->UIAlphaRender(IMAGE->findImage("사용불가능덮개"), ZUISECOND, 100, skillUI[i].left - 4, skillUI[i].top - 4, 180);
    ...
    //현재 선택중인 스킬 표시.
    if (skillnum != 0) {
        ZORDER->UIFrameRender(IMAGE->findImage("스킬이미지"), ZUISECOND, -2, curRc.left, curRc.top-2,
        ...
        if (storage[currentIndex].isCool)
        {
            ...
            //해당박스에 흐릿한 이미지 알파랜더하고
            ZORDER->UIAlphaRender(IMAGE->findImage("사용불가능덮개확대"), ZUISECOND, -1, curRc.left - 4 - 8, curRc.top - 4 - 6, 180);

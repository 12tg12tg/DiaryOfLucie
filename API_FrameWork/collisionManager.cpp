...
void collisionManager::checkUp()
{
	...
	{
		COLORREF  color = GetPixel(IMAGE->findImage("95픽셀")->getMemDC(), PLAYER->getPlayerAddress().x, i);
...
void collisionManager::checkRight()
{
	...
	{
		COLORREF  color = GetPixel(IMAGE->findImage("95픽셀")->getMemDC(), i, PLAYER->getPlayerAddress().y);
